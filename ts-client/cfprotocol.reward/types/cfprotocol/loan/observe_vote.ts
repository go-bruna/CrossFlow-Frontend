...
//   protoc               unknown
// source: cfprotocol/loan/observe_vote.proto
...
/** ObserveVote defines the loan transaction observation entity */
...
function createBaseObserveVote(): ObserveVote {
  return {
    id: 0,
    voter: "",
    txHash: "",
...
export const ObserveVote: MessageFns<ObserveVote> = {
  encode(message: ObserveVote, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    if (message.id !== 0) {
    ...
    if (message.cfTxId !== 0) {
      writer.uint32(88).uint64(message.cfTxId);
    }
    if (message.succeed !== false) {
      writer.uint32(96).bool(message.succeed);
    }
    if (message.txType !== "") {
      writer.uint32(106).string(message.txType);
    }
    if (message.failReason !== "") {
      writer.uint32(114).string(message.failReason);
    }
    if (message.reserved !== "") {
  ...
  decode(input: BinaryReader | Uint8Array, length?: number): ObserveVote {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = createBaseObserveVote();
  ...
  create<I extends Exact<DeepPartial<ObserveVote>, I>>(base?: I): ObserveVote {
    return ObserveVote.fromPartial(base ?? ({} as any));
  },
  fromPartial<I extends Exact<DeepPartial<ObserveVote>, I>>(object: I): ObserveVote {
    const message = createBaseObserveVote();
