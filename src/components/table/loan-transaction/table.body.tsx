	...
	const refineAmount = (amount: string) => {
		return data.collateral_symbol === "USDT"
			...
			: data.collateral_symbol === "BTC"
				? BigNumber(amount).dividedBy(1e8).toNumber()
				: amount;
	};
			...
			<td>
				<div className="flex gap-1 items-center">
					<Typography variant="label-small">
						{refineAmount(data.collateral_amount)}
					</Typography>
					<Typography variant="label-extrasmall" className="text-[#fff]/50">
	...
	return (
		<tbody className="overflow-y-auto">
			{transactions.map((item: IAssetBorrowedTransaction, index: number) => (
				<Row data={item as IAssetBorrowedTransaction} key={index} />
			))}
		</tbody>
	);
};
