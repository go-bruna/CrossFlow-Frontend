...
export const BorrowContainer = (props: Props) => {
  ...
  const { address } = wagmiUseAccount();
	...
	const getLockedBalanceObj = useMemo(() => {
		...
		const _filterBTC = assetProfiles?.find(
			(e: IAssetProfile) => e.symbol === "BTC",
		);
		if (!_filterBTC) return returnValue;

		// find if there's my balances in lock_balance
		const _filteredData = lockData.lock_balance.find(
			(e: IBaseLockBalance) => e.creator === account.bech32Address,
		);

		setActiveLock(_filteredData);
		if (!_filteredData || _filteredData.balances.length < 1) return returnValue;

		// find if there's btc in my lock_balance
		const balanceObj = _filteredData.balances.find(
			(e) => e.asset_id === _filterBTC.id,
		);
		if (!balanceObj) return returnValue;
	...
	const handleBorrow = async () => {
		...
		if (!activeLock)
			return messageApi.Alert({
				...WARNING_MESSAGE,
				content: "There is no any suppliable asset",
			});
		...
		if (!collateralAmount)
			return messageApi.Alert({
				...WARNING_MESSAGE,
				content: "Collateral amount should be greater than 0",
			});
		else if (collateralAmount > calcuateMaxCollateralAmount)
			return messageApi.Alert({
				...WARNING_MESSAGE,
				content: "Collateral amount should be less than borrowabled amount",
			});
		...
		if (!loanAddress || !validateEthereumAddress(loanAddress))
			return messageApi.Alert({
				...WARNING_MESSAGE,
				content: "Loan address should be valid ethereum address.",
			});
		...
		if (calculateDateDiff < 0)
			return messageApi.Alert({
				...WARNING_MESSAGE,
				content: `Duration date should be later than ${dayjs().format("MM/DD/YYY")}`,
			});

		if (calculateDateDiff > Number(loanRateData?.max_duration))
			return messageApi.Alert({
				...WARNING_MESSAGE,
				content: `Duration should be less than max_duration`,
			});

		if (!loanRate || loanRate > Number(loanRateData?.max_loan_rate) * 100)
			return messageApi.Alert({
				...WARNING_MESSAGE,
				content: `Loan rate should be less thatn ${Number(loanRateData?.max_loan_rate) * 100}`,
			});
        ...
        // targetAssetId: Number(props.data.asset_id)
			...
			const res = await client.signAndBroadcast([msg]);
	...
	return (
		<div className="w-full mt-[30px]">
			...
			{/* Loan duration */}
			<div className="mt-5">
				<Typography variant="label-small" className="">
					Loan Duration
				</Typography>
				<DatePicker
					className="mt-3"
				...
				<div className="flex gap-4">
					<Typography variant="label-small" className="mt-2 text-[#36f5cf]/60">
						...
						<span className="text-gray-200">
							{pureNumberFormat(calculateDateDiff ?? 0)}
						</span>
					</Typography>
					<Typography variant="label-small" className="mt-2 text-[#36f5cf]/50">
						Max Duration:{" "}
						<span className="text-gray-200">
							{pureNumberFormat(loanRateData?.max_duration)}
						</span>
					</Typography>
				</div>
			</div>
			...
			{/* Loan Table */}
			<Table.LoanTransaction asset_id={props.data.asset_id} />
		</div>
	);
};
