...
import {
	...
	GET_USDT_SUPPLY_TRANSACTION,
} from "@/constants/query";
import { queryClient } from "@/wagmi";
import { useToast } from "@/hooks/useToast";
import {
	ERROR_MESSAGE,
	FAILED_WALLET_CONNECTION,
	SUCCESS_OPERATION,
	WALLET_INSTALL,
	WARNING_MESSAGE,
...
export const SupplyUSDTContainer = (props: Props) => {
	...
	const { getTokenBalance, approveUSDT } = useWeb3Context();
	...
	const handleSupply = async () => {
		...
		try {
			...
			const approve = await approveUSDT(
				BigNumber(amount).multipliedBy(1e18).toFixed(),
			);
			if (!approve) {
				setLoading(false);
				return;
	...
	return (
		<div className="w-full mt-[30px]">
			...
			<Input.Number
				...
				onMax={() =>
					setRate(
						Number((Number(maxRate?.max_interest_rate ?? 0) * 100).toFixed(0)),
					)
				}
